...
/// Request guard that provides both the authenticated user and the raw JWT token
#[derive(Debug, Clone)]
pub struct AuthenticatedUserWithToken {
...
#[rocket::async_trait]
impl<'r> FromRequest<'r> for AuthenticatedUserWithToken {
    type Error = String;

    async fn from_request(request: &'r Request<'_>) -> Outcome<Self, Self::Error> {
        tracing::debug!("AuthenticatedUserWithToken::from_request called");
        ...
        match jwks_cache.validate_token(token).await {
            Ok(claims) => {
                tracing::info!("Token validated successfully for user: {}", claims.sub);
                Outcome::Success(AuthenticatedUserWithToken {
...
impl<'a> OpenApiFromRequest<'a> for AuthenticatedUserWithToken {
    fn from_request_input(
        _gen: &mut OpenApiGenerator,
        _name: String,
        _required: bool,
